...
clean_motus <- function(edna_file,
  ...
  # Eukaryote superkingdom kept 
  euka_superkingdom <- c("Eukaryota")
  ...
  edna_file_filtered <- edna_file %>%
    ...
    # Remove non eukaryotes taxa
    `if`(remove_not_classif_taxize, filter(., superkingdom_name %in% euka_superkingdom), .) %>%
    # PCR
    group_by(sequence) %>%
...
apply_lulu <- function(edna_file,
  ...
  # Import matchlist
  matchlist <- read.table(paste(path_lulu, "match_list_euka2.2.txt", sep=""), header = F, as.is = T, stringsAsFactors=F)
