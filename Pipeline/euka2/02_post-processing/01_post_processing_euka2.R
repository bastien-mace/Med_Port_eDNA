...
# It cleans and formats data correctly for the euka2 marker
...
# Metadata file 
metadata <- read.table(paste0(swarm_path, "all_samples_euka2.csv"),
...
# Open files 
files <- list.files(swarm_path, pattern = "(.*)euka2(.*)tsv")
...
if(length(grep("Blank(.*)", files, value = T)) == 0){message(paste0("There is no Blank files for the euka2 data"))}
...
if(length(grep("Blank(.*)", files, value = T)) != 0){
  ...
  message(paste0("\nThe Blank threshold for the ", threshold$batch, " batch in the euka2 project is ", threshold$threshold_blank))
}
...
save(list_step1, file = "euka2_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Step 2: Complete taxonomy
# load("euka2_workspace.Rdata")
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
...
save(archive_classif_ncbi, file = "euka2_archive.Rdata")
save(list_step1, list_step2, file = "euka2_workspace.Rdata")
...
if (is.null(data_cleaned[[2]]) == FALSE){
  ...
  species_0_PCR <- list_step3_0PCR %>%
    bind_rows() %>%
    filter(superkingdom_name %in% c("Eukaryota")) %>%
    distinct(sequence, best_identity_database, scientific_name)
  message(paste0("There is ", length(unique(species_0_PCR$scientific_name)), " lost eukaryote taxa during the PCR cleaning step"))
}

save(list_step1, list_step2, list_step3, file = "euka2_workspace.Rdata")
...
write.fasta(as.list(otu_seq$sequence), names = otu_seq$definition, file.out = "OTU_sequences_euka2.fasta", open = "w")
...
# $ makeblastdb -in OTU_sequences_euka2.fasta -parse_seqids -dbtype nucl

# ----- Create the matchlist
# $ blastn -db OTU_sequences_euka2.fasta -outfmt '6 qseqid sseqid pident' -out  match_list_euka2.txt -task megablast -qcov_hsp_perc 80 -perc_identity 84 -query OTU_sequences_euka2.fasta
...
save(list_step1, list_step2, list_step3, list_step4, file = "euka2_workspace.Rdata")
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, file = "euka2_workspace.Rdata")
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, list_step6, file = "euka2_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Write output file
write.table(list_step6, file = "euka2_jaccard.tsv", col.names = NA)
write.table(unique_motu_taxo, file = "euka2_motu_taxo.tsv", col.names = NA)
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 