...
# It cleans and formats data correctly for the teleo marker
...
# Open files 
files <- list.files(swarm_path, pattern = "(.*)teleo(.*)tsv")
...
if(length(grep("Blank(.*)", files, value = T)) == 0){message(paste0("There is no Blank files for the teleo data"))}
...
if(length(grep("Blank(.*)", files, value = T)) != 0){
  ...
  message(paste0("\nThe Blank threshold for the ", threshold$batch, " batch in the teleo project is ", threshold$threshold_blank))
}
...
save(list_step1, file = "teleo_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Step 2: Complete taxonomy
# load("teleo_workspace.Rdata")
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
...
columns_to_remove <- c("amplicon", "OTU", "id", "total", "count",  "cloud", "length", "abundance", "spread", "identity", "taxonomy", "references",
                       "taxid_by_db.db_ncbi_and_custom", "best_match.db_ncbi_and_custom", "match_count.db_ncbi_and_custom", "species_list.db_ncbi_and_custom",
...
save(archive_classif_ncbi, file = "teleo_archive.Rdata")
save(list_step1, list_step2, file = "teleo_workspace.Rdata")
...
data_cleaned <- clean_motus(list_step2, min_PCR = 1) # turn min_PCR to 0 if replicates were pooled
...
if (is.null(data_cleaned[[2]]) == FALSE){
  ...
  message(paste0("There is ", length(unique(species_0_PCR$scientific_name)), " lost fish taxa during the PCR cleaning step"))
}

save(list_step1, list_step2, list_step3, file = "teleo_workspace.Rdata")
...
write.fasta(as.list(otu_seq$sequence), names = otu_seq$definition, file.out = "OTU_sequences_teleo.fasta", open = "w")
...
# $ makeblastdb -in OTU_sequences_teleo.fasta -parse_seqids -dbtype nucl

# ----- Create the matchlist
# $ blastn -db OTU_sequences_teleo.fasta -outfmt '6 qseqid sseqid pident' -out  match_list_teleo.txt -task megablast -qcov_hsp_perc 80 -perc_identity 84 -query OTU_sequences_teleo.fasta
...
save(list_step1, list_step2, list_step3, list_step4, file = "teleo_workspace.Rdata")
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, file = "teleo_workspace.Rdata")
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, list_step6, file = "teleo_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Write output file
write.table(list_step6, file = "teleo_jaccard.tsv", col.names = NA)
write.table(unique_motu_taxo, file = "teleo_motu_taxo.tsv", col.names = NA)
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 