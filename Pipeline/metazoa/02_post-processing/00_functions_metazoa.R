...
add_classification <- function(edna_file,
  ...
  if(is.null(archive_file)){
    archive_file <- setNames(data.frame(matrix(ncol = 15, nrow = 0)), c("kingdom", "kingdom_name",
                                                                        "phylum", "phylum_name",
                                                                        "class", "class_name",
                                                                        "order", "order_name",
                                                                        "family", "family_name",
  ...
  for (i in taxa_list){
    ...
    taxon_classification <- do.call(cbind, lapply(c(taxon_kingdom,
                                                    taxon_phylum,
                                                    taxon_class,
                                                    taxon_order,
                                                    taxon_family,
...
clean_motus <- function(edna_file,
  ...
  # Animal kingdom kept 
  animal_kingdom <- c("Metazoa")
  
  # Verification that the kingdom column exists
  if(remove_not_classif_taxize & !('kingdom_name' %in% colnames(edna_file)) ) stop(message("Error: The column 'kingdom_name' does not exist. Please use the 'add_classif_name' function before or set the 'remove_not_classif_taxize' option to FALSE."))
  ...
  edna_file_filtered <- edna_file %>%
    ...
    # Remove non animal taxa
    `if`(remove_not_classif_taxize, filter(., kingdom_name %in% animal_kingdom), .) %>%
    # PCR
    group_by(sequence) %>%
...
apply_lulu <- function(edna_file,
  ...
  # Import matchlist
  matchlist <- read.table(paste(path_lulu, "match_list_metazoa.txt", sep=""), header = F, as.is = T, stringsAsFactors=F)
