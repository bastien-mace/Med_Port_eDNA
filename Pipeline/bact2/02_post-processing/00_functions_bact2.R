...
add_classification <- function(edna_file,
  ...
  if(is.null(archive_file)){
    archive_file <- setNames(data.frame(matrix(ncol = 17, nrow = 0)), c("superkingdom", "superkingdom_name",
  ...
  for (i in taxa_list){
    ...
    # Add superkingdom
    taxon_superkingdom <- taxon_classification_complete %>%
      filter(rank == "superkingdom") %>%
      rename(superkingdom_name = name) %>%
      rename(superkingdom = id) %>%
      select(superkingdom, superkingdom_name)
    
    # Add kingdom
    taxon_kingdom <- taxon_classification_complete %>%
      filter(rank == "kingdom") %>%
      rename(kingdom_name = name) %>%
      rename(kingdom = id) %>%
      select(kingdom, kingdom_name)
    
    # Add phylum
    taxon_phylum <- taxon_classification_complete %>%
      filter(rank == "phylum") %>%
      rename(phylum_name = name) %>%
      rename(phylum = id) %>%
      select(phylum, phylum_name)
    ...
    taxon_classification <- do.call(cbind, lapply(c(taxon_superkingdom,
                                                    taxon_kingdom,
                                                    taxon_phylum,
                                                    taxon_class,
                                                    taxon_order,
...
clean_motus <- function(edna_file,
                        ...
                        remove_chimera = TRUE,
                        remove_not_classif_taxize = FALSE){

  # Prokaryote superkingdom kept 
  proka_superkingdom <- c("Bacteria", "Archaea")
  
  # Verification that the class column exists
  if(remove_not_classif_taxize & !('superkingdom_name' %in% colnames(edna_file)) ) stop(message("Error: The column 'superkingdom_name' does not exist. Please use the 'add_classif_name' function before or set the 'remove_not_classif_taxize' option to FALSE."))
  ...
  edna_file_filtered <- edna_file %>%
    ...
    # Remove non prokaryotes taxa
    `if`(remove_not_classif_taxize, filter(., superkingdom_name %in% proka_superkingdom), .) %>%
    # PCR
    group_by(sequence) %>%
...
apply_lulu <- function(edna_file,
  ...
  # Import matchlist
  matchlist <- read.table(paste(path_lulu, "match_list_bact2.2.txt", sep=""), header = F, as.is = T, stringsAsFactors=F)
