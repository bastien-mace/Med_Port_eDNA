...
# It cleans and formats data correctly for the bact2 marker
...
# Metadata file 
metadata <- read.table(paste0(swarm_path, "all_samples_bact2.csv"),
...
# Open files 
files <- list.files(swarm_path, pattern = "(.*)bact2(.*)tsv")
...
if(length(grep("Blank(.*)", files, value = T)) == 0){message(paste0("There is no Blank files for the bact2 data"))}
...
if(length(grep("Blank(.*)", files, value = T)) != 0){
  ...
  message(paste0("\nThe Blank threshold for the ", threshold$batch, " batch in the bact2 project is ", threshold$threshold_blank))
}
...
save(list_step1, file = "bact2_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Step 2: Complete taxonomy
# load("bact2_workspace.Rdata")
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
...
columns_to_remove <- c("amplicon", "OTU", "id", "total", "count",  "cloud", "length", "abundance", "spread", "identity", "taxonomy", "references",
                       "taxid_by_db.db_silva_138.1_ssuref_nr_full", "best_match.db_silva_138.1_ssuref_nr_full", "match_count.db_silva_138.1_ssuref_nr_full", "species_list.db_silva_138.1_ssuref_nr_full",
...
save(archive_classif_ncbi, file = "bact2_archive.Rdata")
save(list_step1, list_step2, file = "bact2_workspace.Rdata")
...
if (is.null(data_cleaned[[2]]) == FALSE){
  ...
  species_0_PCR <- list_step3_0PCR %>%
    bind_rows() %>%
    filter(superkingdom_name %in% c("Bacteria", "Archaea")) %>%
    distinct(sequence, best_identity_database, scientific_name)
  message(paste0("There is ", length(unique(species_0_PCR$scientific_name)), " lost prokaryote taxa during the PCR cleaning step"))
}

save(list_step1, list_step2, list_step3, file = "bact2_workspace.Rdata")
...
write.fasta(as.list(otu_seq$sequence), names = otu_seq$definition, file.out = "OTU_sequences_bact2.fasta", open = "w")
...
# $ makeblastdb -in OTU_sequences_bact2.fasta -parse_seqids -dbtype nucl

# ----- Create the matchlist
# $ blastn -db OTU_sequences_bact2.fasta -outfmt '6 qseqid sseqid pident' -out  match_list_bact2.txt -task megablast -qcov_hsp_perc 80 -perc_identity 84 -query OTU_sequences_bact2.fasta
...
save(list_step1, list_step2, list_step3, list_step4, file = "bact2_workspace.Rdata")
...
samples_to_keep <- unique(grep(
  "SPY213513|SPY213520|SPY221741|SPY221742|SPY200468|SPY200474|SPY181699|SPY192310|SPY181149|SPY181150|SPY213512|SPY213515|SPY213516|SPY221745|SPY221746|SPY200806|SPY200807|SPY213514|SPY213517|SPY221749|SPY221750|SPY221751|SPY221753|SPY202505|SPY202519|SPY202520|SPY201239|SPY201241|SPY213519|SPY213527|SPY221747|SPY221748|SPY213523|SPY213525|SPY221743|SPY221744|SPY213518|SPY213521|SPY213522|SPY221739|SPY221740|SPY201216|SPY201263",
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, file = "bact2_workspace.Rdata")
...
# Remove PCR identifiers
occurences_table$sample_name <- gsub("_.*", "", occurences_table$sample_name)
...
save(list_step1, list_step2, list_step3, list_step4, list_step5, list_step6, file = "bact2_workspace.Rdata")

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 
# Write output file
write.table(list_step6, file = "bact2_jaccard.tsv", col.names = NA)
write.table(unique_motu_taxo, file = "bact2_motu_taxo.tsv", col.names = NA)
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- # 