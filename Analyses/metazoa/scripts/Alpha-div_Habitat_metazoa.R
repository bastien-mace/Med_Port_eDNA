...
richness_table <- read.csv2("../data/metazoa_richness.csv") %>%
...
boxplot(residuals(model_REML) ~ richness_table$Habitat, 
        varwidth = TRUE,
        ylab = "Residuals",
        xlab = "Habitat")
abline(h = 0)
...
richness_table[richness_table == "Marina_Summer"] <- "Marina\n(n = 12)"
richness_table[richness_table == "Reserve"] <- "Reserve\n(n = 8)"
...
# ggsave("../plots/Alpha-div_Habitat_metazoa_predicted.tiff", alpha_div_predicted, dpi = 600, width = 15, height = 15, units = "cm")
...
# ggsave("../plots/Alpha-div_Habitat_metazoa.tiff", alpha_div, dpi = 600, width = 15, height = 15, units = "cm")
