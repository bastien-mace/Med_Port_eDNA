...
teleo_richness <- read.csv2("../data/teleo_richness_marina.csv") %>%
  as.data.frame() %>%
  rename("teleo" = "Richness")

metazoa_richness <- read.csv2("../data/metazoa_richness_marina.csv") %>%
  as.data.frame() %>%
  rename("metazoa" = "Richness")

euka2_richness <- read.csv2("../data/euka2_richness_marina.csv") %>%
  as.data.frame() %>%
  rename("euka2" = "Richness")

bact2_richness <- read.csv2("../data/bact2_richness_marina.csv") %>%
  as.data.frame() %>%
...
bp[[4]]
dev.off()
barfig5 <- image_graph(width = 350, height = 350, res = 300, bg = 'transparent')
bp[[5]]
dev.off()
barfig6 <- image_graph(width = 350, height = 350, res = 300, bg = 'transparent')
bp[[6]]
dev.off()

# Add them to the map
final <- image_composite(fig, barfig6, offset = "+200+1125")
final <- image_composite(final, barfig2, offset = "+100+800")
final <- image_composite(final, barfig5, offset = "+425+750")
final <- image_composite(final, barfig3, offset = "+750+850")
final <- image_composite(final, barfig4, offset = "+1100+925")
final <- image_composite(final, barfig1, offset = "+1400+625")
final

# ggsave("../plots/Alpha-div_Map_marina.tiff", image_ggplot(final), width = 2400, height = 2200, unit = "px", dpi = 600)
