...
# Extract MOTU order names
motu_order <- read.csv2("../data/all_motu_taxo.csv") %>%
  select(c("order_name")) %>%
  na.omit() %>%
  unique()
motu_order

# Extract MOTUs corresponding to these order
motu_table <- read.csv2("../data/all_motu_taxo.csv") %>%
  filter(order_name %in% motu_order$order_name) %>%
  select(c("definition", "order_name"))
...
# Build a matrix with the number of MOTUs belonging to each order in each site
occurence_order <- data.frame(matrix(ncol = length(colnames(occurence_motu)) - 1, nrow = length(motu_order$order_name)))
colnames(occurence_order) <- c(colnames(occurence_motu)[1:length(colnames(occurence_motu)) - 1])
rownames(occurence_order) <- motu_order$order_name

## Calculate the abundance of each order in each site
for (i in motu_order$order_name){
  occurence_order_i <- occurence_motu %>%
    filter(order_name == i)
  occurence_order[i, ] <- colSums(occurence_order_i[, -c(dim(occurence_order_i)[2])])
}


# Prepare data for dbRDA
biodiv_table <- occurence_order %>%
  t(.) %>%
  as.data.frame() %>%
...
## Select the Ys
data_dbRDA_order <- biodiv_table %>%
  select(-c("Site", "Habitat", "Season", "Longitude", "Latitude"))

## Select the Xs
meta_dbRDA_order <- biodiv_table %>%
  select(c("Site", "Habitat", "Season", "Longitude", "Latitude"))

meta_dbRDA_order$Longitude <- as.numeric(meta_dbRDA_order$Longitude)
meta_dbRDA_order$Latitude <- as.numeric(meta_dbRDA_order$Latitude)

row.names(data_dbRDA_order) <- meta_dbRDA_order$Site

## Remove absent MOTUs (those only present in reserves)
data_dbRDA_order <- data_dbRDA_order[, colSums(data_dbRDA_order) != 0]


# Set the model
dbRDA_order <- capscale(data_dbRDA_order ~ Season + Condition(Longitude + Latitude),
                         meta_dbRDA_order, distance ='bray')

# Get the inertia statistics                   
dbRDA_order

# Test the significance of the model
anova.cca(dbRDA_order, permutations = 9999)

# Calculate the adjusted-R²
RsquareAdj(dbRDA_order)


# Extract dbRDA data
sumdbRDA <- summary(dbRDA_order)

# Get the scores
scrs <- scores(dbRDA_order, display = c("species", "sites", "bp"), scaling = 2)
...
# ggsave("../plots/Beta-div_dbRDA_Season_order_sites.tiff", dbRDA_site_scores, dpi = 600, width = 20, height = 15, units = "cm")
...
## Create a table with the markers and their corresponding orders
markers_order <- motu_table %>%
  rename("SPECIES" = "order_name")
markers_order$definition <- gsub("teleo.*", "teleo", markers_order$definition)
markers_order$definition <- gsub("metazoa.*", "metazoa", markers_order$definition)
markers_order$definition <- gsub("euka2.*", "euka2", markers_order$definition)
markers_order$definition <- gsub("bact2.*", "bact2", markers_order$definition)

### Dereplicate the data
duplicate_cols <- duplicated(markers_order)
markers_order_unique <- markers_order[!duplicate_cols, ]

species_centroids_10 <- inner_join(species_centroids_10, markers_order_unique)
...
# ggsave("./plots/Beta-div_dbRDA_Habitat_order_species.tiff", dbRDA_species_scores, dpi = 600, width = 20, height = 15, units = "cm")