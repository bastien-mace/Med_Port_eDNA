...
jaccard <- read.csv2("../data/euka2_jaccard.csv")
...
## Remove absent MOTUs (those only present in marinas in reserves)
data_dbRDA <- data_dbRDA[, colSums(data_dbRDA) != 0]


# Set the model
dbRDA <- capscale(data_dbRDA ~ Season + Condition(Longitude + Latitude),
...
sites_centroids$SITE <- gsub("*.Autumn", "", sites_centroids$SITE)
...
# ggsave("../plots/Beta-div_dbRDA_Season_euka2.tiff", dbRDA_site_scores, dpi = 600, width = 20, height = 15, units = "cm")
